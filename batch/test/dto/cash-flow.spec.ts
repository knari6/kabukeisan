...
describe("CashFlowDto", () => {
  ...
  beforeAll(() => {
    financialData = {
      ...
      cashFlowStatement: {
        netCashProvidedByOperatingActivities: random.randomInt(
          1000000,
          9999999
        ),
        netCashProvidedByInvestingActivities: random.randomInt(
          1000000,
          9999999
        ),
        netCashProvidedByFinancingActivities: random.randomInt(
          1000000,
          9999999
        ),
    ...
    cashFlowDto = new CashFlowDto(financialData);
  });
  describe("dto", () => {
    it("CashFlowDtoを作ること", () => {
      ...
      expect(dto.operatingCf).toEqual(
        financialData.cashFlowStatement.netCashProvidedByOperatingActivities
      );
      expect(dto.investingCf).toEqual(
        financialData.cashFlowStatement.netCashProvidedByInvestingActivities
      );
      expect(dto.financialCf).toEqual(
        financialData.cashFlowStatement.netCashProvidedByFinancingActivities
      );
    });
  });
});
